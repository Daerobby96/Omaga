{{-- resources/views/mahasiswa/logbook/create.blade.php --}}
@extends('layouts.app')
@section('title','Isi Logbook')
@section('page-title','Isi Logbook Harian')
@section('page-sub', $pengajuanAktif->mitra->nama_perusahaan . ' · ' . now()->locale('id')->translatedFormat('d F Y'))
...
<div class="card mb-3" style="background:#e3f8ef;border:1px solid #a7f3d0;">
    <div class="card-body d-flex align-items-center gap-3 py-3">
        ...
        <div>
            <div style="font-size:14px;font-weight:700;color:#065f46;">{{ $pengajuanAktif->mitra->nama_perusahaan }}</div>
            <div style="font-size:12px;color:#047857;">Dosen: {{ $pengajuanAktif->dosen?->nama_lengkap ?? '-' }} · {{ $pengajuanAktif->bidang_kerja }}</div>
        </div>
    </div>
</div>
            ...
            <div class="row g-3">
                ...
                <div class="col-md-4">
                    ...
                    <input type="time" name="jam_masuk" class="form-control @error('jam_masuk') is-invalid @enderror"
                           value="{{ old('jam_masuk','08:00') }}" step="1800">
                ...
                <div class="col-md-4">
                    ...
                    <input type="time" name="jam_keluar" class="form-control @error('jam_keluar') is-invalid @enderror"
                           value="{{ old('jam_keluar','17:00') }}" step="1800">
...
- Review pull request dari rekan kerja">{{ old('kegiatan') }}</textarea>
                @error('kegiatan')<div class="invalid-feedback">{{ $message }}</div>@enderror
                <div class="d-flex justify-content-end mt-1">
                    <span id="kegiatanCount" style="font-size:11px;color:#94a3b8;">0 karakter</span>
                </div>
            </div>
            <div class="mb-3">
                <label class="form-label">Hasil / Output</label>
                <textarea name="hasil" class="form-control" rows="3"
                          placeholder="Apa yang berhasil Anda selesaikan atau capai hari ini?">{{ old('hasil') }}</textarea>
            </div>
            <div class="mb-3">
                <label class="form-label">Kendala (jika ada)</label>
                <textarea name="kendala" class="form-control" rows="2"
                          placeholder="Apakah ada kendala yang dihadapi? Bagaimana solusinya?">{{ old('kendala') }}</textarea>
            </div>
            <div>
                <label class="form-label">Foto Kegiatan (opsional)</label>
                <input type="file" name="foto_kegiatan" class="form-control" accept="image/*" id="fotoInput">
...
$(document).ready(function(){
    ...
    // Counter karakter
    $('textarea[name="kegiatan"]').on('input',function(){
        const n = $(this).val().length;
        $('#kegiatanCount').text(n+' karakter').css('color', n < 20 ? '#ef4444' : '#94a3b8');
    });
